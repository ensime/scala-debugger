...
import org.scaladebugger.repl.language.parsers.grammar.ReservedKeywords
  ...
  describe("DebuggerParser") {
    describe("functions") {
      they("should support parsing definitions") {
        ...
        parseFirstAsExpression(s"$f(){}") should be (InterpretedFunction(Nil, ExpressionGroup(Nil)))
        parseFirstAsExpression(s"$f() {}") should be (InterpretedFunction(Nil, ExpressionGroup(Nil)))
        parseFirstAsExpression(s"$f (){}") should be (InterpretedFunction(Nil, ExpressionGroup(Nil)))
        parseFirstAsExpression(s"$f () {}") should be (InterpretedFunction(Nil, ExpressionGroup(Nil)))
        parseFirstAsExpression(s"$f(a){}") should be (InterpretedFunction(Seq(Identifier("a")), ExpressionGroup(Nil)))
        parseFirstAsExpression(s"$f( a ){}") should be (InterpretedFunction(Seq(Identifier("a")), ExpressionGroup(Nil)))
        parseFirstAsExpression(s"$f(a,b){}") should be (InterpretedFunction(Seq(Identifier("a"), Identifier("b")), ExpressionGroup(Nil)))
        parseFirstAsExpression(s"$f( a , b ){}") should be (InterpretedFunction(Seq(Identifier("a"), Identifier("b")), ExpressionGroup(Nil)))
        parseFirstAsExpression(s"$f(){a;b}") should be (InterpretedFunction(Nil, ExpressionGroup(Seq(Identifier("a"), Identifier("b")))))
      }
      ...
      they("should support parsing invocations using only whitespace") {
        ...
        val context1 = Context(Scope.newRootScope(Map(
          Identifier(f) -> InterpretedFunction(Seq(Identifier("arg")), null)
        )))
        ...
        val context2 = Context(Scope.newRootScope(Map(
          Identifier(f) -> InterpretedFunction(Seq(Identifier("arg"), Identifier("")), null)
        )))
        ...
        val context3 = Context(Scope.newRootScope(Map(
          Identifier(f) -> InterpretedFunction(Seq(Identifier(""), Identifier("arg")), null)
        )))
      ...
      they("should support parsing assignment of definitions to variables") {
        ...
        parseFirstAsExpression(s"$n:=$f(){}") should be (Variable(Identifier(n), InterpretedFunction(Nil, ExpressionGroup(Nil))))
        parseFirstAsExpression(s"$n := $f () {}") should be (Variable(Identifier(n), InterpretedFunction(Nil, ExpressionGroup(Nil))))
        parseFirstAsExpression(s"$n := $f ( ) { }") should be (Variable(Identifier(n), InterpretedFunction(Nil, ExpressionGroup(Nil))))
      }
    }
                      ...
                      Multiply(
                        Multiply(
                          Multiply(
                            ...
                            InterpretedFunction(Nil, ExpressionGroup(Nil))
                          ),
    ...
    describe("conditionals") {
      they("should support parsing if statements") {
        ...
        parseFirstAsExpression(s"$i(a)a") should be (Conditional(Identifier("a"), Identifier("a"), NoOp()))
        parseFirstAsExpression(s"$i(a){a}") should be (Conditional(Identifier("a"), ExpressionGroup(Seq(Identifier("a"))), NoOp()))
      }
